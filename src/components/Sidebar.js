...
const Sidebar = ({menu}) => {
  ...
  return (
    ...
    <div className="sidebar fixed top-0 left-0 h-screen main-bg text-lg">
      ...
      <ul className="space-y-2 px-4">
        <li onClick={() => loadPage('/dashboard', 'dashboard')} className={`menu-item cursor-pointer ${activeMenu === 'dashboard' ? 'active' : ''}`}>
          <button className="flex items-center">
            <FontAwesomeIcon icon={faGauge} className="mr-2" /> Dashboard
          </button>
        </li>
        
        <li onClick={() => loadPage('/reservations', 'reservations')} className={`menu-item cursor-pointer ${activeMenu === 'reservations' ? 'active' : ''}`}>
          <button className="flex items-center">
            <FontAwesomeIcon icon={faCalendar} className="mr-2" /> Reservations
          </button>
        </li>
        
        <li onClick={() => loadPage('/checkin', 'checkin')}  className={`menu-item cursor-pointer ${activeMenu === 'checkin' ? 'active' : ''}`}>
          <button className="flex items-center">
            <FontAwesomeIcon icon={faDoorOpen} className="mr-2" /> Check-In/Check-Out
          </button>
        </li>
        
        <li onClick={() => loadPage('/guestmanagement', 'guest-management')} className={`menu-item cursor-pointer ${activeMenu === 'guest-management' ? 'active' : ''}`}>
          <button className="flex items-center">
            <FontAwesomeIcon icon={faUsersGear} className="mr-2" /> Guest Management
          </button>
        </li>
        
        <li onClick={() => loadPage('/billing', 'billing')} className={`menu-item cursor-pointer ${activeMenu === 'billing' ? 'active' : ''}`}>
          <button className="flex items-center">
